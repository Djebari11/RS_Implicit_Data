...
data_rating_frequency %>% 
  ...
  labs(x = "Note", y = "Fréquence absolue") +
  theme_classic()+
...
#DZ map for purchase fequency
```{r}
library(sp)
library(sf)


dz_mapw <- raster::getData("GADM", country ="DZ", level = "1")
dz_map <- raster::getData("GADM", country ="DZ", level = "0")


ggplot() + 
  geom_sf(data = dz_mapw %>% st_as_sf(), fill = 'white') + 
  coord_sf(ylim = c(35,38), xlim = c(-2.5, 8.5)) +
  geom_point(data = data2,
             aes(x = delivery_address_loc_lng, y = delivery_address_loc_lat),
             col = "red") +
  theme_bw()
```
